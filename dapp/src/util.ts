...
export const createEnginePayloads = (engine: Micropolis) => {
    ...
    const populationDensity = Uint16ArrayToHex(engine.populationDensity);
    // const category = numberToHex(engine.category, { size: 4 });
    // const level = numberToHex(engine.level, { size: 4 });
    return { map, population, totalFunds, cityTime, cityTax, taxFund, roadPercent, roadFund, firePercent, fireFund, policePercent, policeFund, score, value, scoreDelta, populationDelta, populationDensity };
};
